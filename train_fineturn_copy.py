# 计划：1.修改list和训练可能存在的问题 2.搞清楚baseline的训练集和测试集应该没问题  阅读DP和DDP的代码，保证能跑，无问题 3.阅读fineturning代码
# 计划：1.高明白baseline中fineturning的方法 2.写代码，复现baseline
...
def get_pretrain(pretrain_path, n_class):
	...
	# n_class = n_class, m = 0.2, s = 30
	classifier = AAMsoftmax(n_class, 0.25, 30)
	...
	for name, param in model.named_parameters():
		param.requires_grad = ( name.startswith('attention') or name.startswith('bn5') or name.startswith('fc6')or name.startswith('bn6'))  #or name.startswith('bn1'))
...
def save_model(save_path, epoch, model, classifier, optimizer, scheduler):
    torch.save({'model': model.state_dict(),
            ...
            }, os.path.join(save_path, 'model_v1_b%d.pkl' % epoch))
...
def train(model, train_dataloader, classifier, optimizer, scheduler, hyperparameters, epochs):
	...
	for epoch in range(0,epochs):
		...
		for i, (feats, key) in enumerate(train_dataloader, start = 1):
			...
			sys.stderr.flush()
		sys.stdout.write("\n")
		save_model("/home/zhy/FFSVC/exps/exp_best", epoch, model, classifier, optimizer, scheduler)
...
def main():
	...
	train_dataset = train_loader("/home/wldong/data/train_list_re.txt","/home/wldong/ffsvc",
					"/home/wldong/ffsvc/musan/musan_cut_4","/home/wldong/ffsvc/rirs/RIRS_NOISES/simulated_rirs",
					200)
	...
	model, classifier = get_pretrain("/home/zhy/FFSVC/exps/exp_best/model_v1_a2.pkl",7325)
	...
	train(model, train_dataloader, classifier, optimizer, scheduler, hyperparameters,20)
