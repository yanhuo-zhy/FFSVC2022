...
def get_score(embd_dict, trial_file):
    ...
    with open(trial_file) as fh:
        for line in fh:
            ...
            result =  1 - spatial.distance.cosine(embd_dict[utt1], embd_dict[utt2])#cosin_dis_advance(embd_dict[utt1], embd_dict[utt2])
            score.append(result)
    return score

def cosin_dis_advance(ref, com):
    """
    ...
    def cosine_with_adaptive(ref, com):
        ...
        score = (np.dot((ref - com_mean).T, (com - com_mean))) / (
                    np.linalg.norm(np.dot(com_sqrt_cov, ref)) * np.linalg.norm(np.dot(com_sqrt_cov, com)))
        return score
